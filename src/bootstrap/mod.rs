...
use std::{time::Duration, sync::{Arc, atomic::{AtomicBool, Ordering}}, convert::TryFrom};
...
impl BootstrapProtocol {
    pub fn new(peer_store: PeerStore, peer_id: PeerId, local_multiaddr: Multiaddr, is_server: bool, event_tx: Sender<Event>) -> Self {
        BootstrapProtocol {
            peer_store,
    ...
    pub async fn publish_ourself(_ctx: Context, peer_id: PeerId, local_multiaddr: Multiaddr, w_stream: WriteStream) -> Result<(), Error> {
        ...
        let encoded_ourself = {
            ...
            ourself.encode(&mut encoded_ourself).context("encode ourself")?;

            encoded_ourself.freeze()
        };

        w_stream.lock().await.send(encoded_ourself).await.context("publish ourself")?;
        Ok(())
    }

    pub async fn push_peers(_ctx: Context, peer_store: PeerStore, w_stream: &mut WriteStream) -> Result<(), Error> {
        let peer_infos = peer_store.choose(40).await.into_iter().map(PeerInfo::new).collect();
        ...
        let encoded_peers = {
            ...
            peers.encode(&mut encoded_peers).context("encode push peers")?;

            encoded_peers.freeze()
        };

        w_stream.lock().await.send(encoded_peers).await.context("push peers")?;
        Ok(())
    }

    // TODO: set interval through ctx
    pub async fn interval_push_peers(ctx: Context, peer_store: PeerStore, mut w_stream: WriteStream) -> Result<(), Error> {
        loop {
    ...
    pub async fn new_arrived(_ctx: Context, peer_store: PeerStore, r_stream: ReadStream, mut ev_tx: Sender<Event>) -> Result<(), Error> {
        let encoded_peers = r_stream.lock().await.try_next().await?.ok_or(BootstrapError::NoNewArrive)?;
        ...
        ev_tx.try_send(Event::NewArrived).context("impossible, try send new arrived event")?;
    ...
    async fn handle(&self, stream: FramedStream) {
        ...
        if !self.is_server.load(Ordering::SeqCst) {
            ...
            tokio::spawn(async move {
                if let Err(err) = Self::publish_ourself(Context::new(), peer_id, local_multiaddr, w_stream).await {
            ...
            tokio::spawn(async move {
                if let Err(err) = Self::interval_push_peers(Context::new(), peer_store, w_stream).await {
